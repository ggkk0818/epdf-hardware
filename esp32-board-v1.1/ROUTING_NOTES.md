...
**第十一轮（2026-09-18，依 `..._USB_SI_Freeze_and_GND_Final_Cleanup_Plan.md`）**——
USB 冻结为 **Plan C**；GND 继续按 record 逐条分类。

**USB（Checkpoint I）：Plan A「F.Cu 0 过孔差分对」实测走不通 —— 用洪水填充给出了硬证据。**

```text
把 DN 的 3 过孔绕行整段删掉后，从 D3.1 焊盘的栅格做 F.Cu 洪水填充：
F.Cu 口袋只有 1982 格，bbox x 18.0..26.3  y 71.6..79.0
  → 向北在 y≈71.6 就被封死，根本到不了 R10.1（R10.1 侧的口袋是整板 172k 格）
封死它的是 USB_SHIELD 的扇出：
  (17.00,74.70)->(20.50,71.20) 斜线 + (20.50,71.20)->(22.30,71.20) 横线
  + 串在其中的 R8.1 焊盘 (22.63,71.16)，其焊盘 x 22.23..23.03
  —— 正好压在 MD 建议的 DN 走廊 x≈22.98 上。
```

MD §4 只允许动 `USB_CC1 局部 Via` 与 `BAT_BUS 局部换层点`，而真正的阻塞物是
**USB_SHIELD / R8 的扇出**（不在允许清单内）。按 MD §13 的止损条款，不再撬动电源骨架：
**USB 冻结为 Plan C**（DP 全 F.Cu / 0 过孔；DN 走 F.Cu→In2→B.Cu、3 过孔），
并把 DN 绕行恢复回去，保持 `Non-GND = 0`。
> 若要升级为 Plan B（DP/DN 对称换层）或真正 0 过孔，需要先动 USB_SHIELD/R8 的扇出，
> 属于 MD 未授权的范围，需另行确认。

**GND（Checkpoint J）：按 record 逐条分类（18 条 = 11 条 Zone/Plane + 4 条 Pad + 3 条 Track）**。

本轮已处理：删除 U3.4 附近的历史短桩（8 段重复残段，`gnd_fix`），无新增违规；
record 组合随之变化（原 `Via↔Track` 变为 `Pad4[U3]↔Via`），总数仍 18。

各 Pad record 的实测卡点：

| Record | 实测结论 |
|---|---|
| `R13.2 ↔ U2.25`、`R13.2 ↔ 短铜` | R13.2 的 F.Cu 口袋只有 **118 格**（x 29.8~31.0, y 61.4~62.7），被 I2C_SDA/I2C_SCL/CHG 扇出封死 → MD §21 的「短直线接 U2 Pad25」实际画不过去；需要先局部 rip I2C_SDA 的一小段 |
| `U5.3 ↔ U5.5`（两处，= Track↔Track 记录） | U5.3 的短桩末端 (28.70,70.12) F.Cu 口袋是**整板级**（199k 格）→ 补一段短铜接铺铜即可；U5.5 的短桩末端 (29.73,70.69) 口袋只有 **79 格**、且 0.4/0.2 过孔放不下 → 需要局部让路 |
| `J3.SH ↔ 短铜` | J3.SH 自己已有 0.6/0.3 过孔（接地正常），残留的是一段 0.2828 mm 旧短铜 |
| 11 条 Zone/Plane | F.Cu 铺铜块之间 / 与 In1 平面 / 与 In2、B.Cu 铺铜的孤立铜 → 按 MD §27 逐块判断「删孤岛 / 补缝合过孔」 |

检查点：`esp32-board-v1.1_gnd_round2_20260918.kicad_pcb`。

**第十二轮（2026-09-18，依 `..._GND_Final_Convergence_and_Final_DRC_Plan.md`）**——
USB 正式冻结为 Plan C；GND 按 MD §26 的单步清单开始逐项清零（每步 DRC 验证）。

| 步骤 | 处理 | 结果 |
|---|---|---|
| §3 ① U3 Pad4 (32.85,45.75) → 已有 GND Via (33.80,45.00) | 加 3 段 **0.20 mm** F.Cu：(33.80,45.00)→(33.60,45.00)→(33.60,45.75)→(32.95,45.75)，全部落在实测可走格上，**不加过孔** | ✅ U3.4 record 消失，无新违规 |
| §2 ⑯ 删除真孤岛 | 删掉 (43.1078,64.575)→(43.1078,64.8578) 的 0.2828 mm 历史短桩（2 段重复，无 Pad / 无 Via，属 MD §16.1 A 类） | ✅ J3.SH 相关 record 消失 |
| 小结 | **GND 18 → 17**（11 Zone/Plane + 3 U5 短桩 + 2 R13.2 + 1 C38.2），**DRC 0/0** | |

剩余项的下一步（全部已定位，按 MD §26 顺序）：

```text
U5.3  (28.70,70.12)：F.Cu 口袋整板级 → 补短铜接 GND 铺铜即可
U5.5  (29.73,70.69)：口袋仅 79 格 → 按 MD §6~§8 先局部 rip USB_VBUS_DET 的
                      (29.3,70.6)~(30.5,71.8) 一小段并手工向外推，再给 U5.5 短铜
R13.2 (30.11,61.86)：口袋 118 格 → 按 MD §9~§11 先局部 rip I2C_SDA 约 1~3 mm，
                      再画 0.20 mm 短线接到 U2 Pad25，随后恢复 I2C_SDA
C38.2 ：新出现的 Pad record（Refill 后铺铜边界变化），按 MD §16.2 补 1 颗缝合过孔
11 条 Zone/Plane：按 MD §15~§16 先生成只读的 GND component report
                      （Layer / BBox / 面积 / 是否含 Pad / Via / 是否接 In1 / 关联 record），
                      再按 A（删孤岛）/ B（补 1 颗缝合过孔）/ C（Refill 或删除碎片）处理
```
...
**第十九轮（2026-09-18，用户选定 U5.5 的 Plan B）**：

| 步骤 | 处理 | 结果 |
|---|---|---|
| ① 局部 rip `USB_VBUS_DET` | 按 MD §13 只拆 U5 侧那一小段（4 段，`(30.5,71.5)~(29.3,70.6)` 的包绕） | ✅ 拆掉后该区域腾空 |
| ② 手工改道 `USB_VBUS_DET` | 从 `(30.50,71.50)` 向南绕行：`→(30.50,72.30)→(29.30,72.30)→(29.30,70.60)→U5.4`；竖向段按净空放在 **x=29.30** 的合法窗口（U5.3 焊盘 29.0 与 U5.5 焊盘 29.6 之间） | ✅ `USB_VBUS_DET` 仍 Connected（DRC 无相关 record），0 违规 |
| ③ 接 U5.5 | U5.5 的短桩沿东侧空走廊南下：`(29.73,70.69)→(29.73,71.95)`，末端落在 F.Cu GND 铺铜内（`pour_hit` 确认） | 该处铺铜本身仍是孤立铜块，record 未消 |
| ④ 复查分量地图 | `gnd_components.py`：MAIN_GND + **3 个孤立铜块**（U5.11 14 格、U5.10 14 格、U5.5 151 格）；`find_stitch_candidates.py` 对这三块**都没有候选**——它们只是 U5 引脚旁的**细条铺铜**，0.4/0.20 过孔塞不进去 | ✗ 仍需"先把铜加宽再落孔" |

当前：**unconnected = 2（都在 U5）/ DRC 0 Error / 0 Warning / Non-GND = 0**。
检查点：`esp32-board-v1.1_gnd_round8_20260918.kicad_pcb`。

下一步（本轮新结论）：这三块都是"细条"铜，正确做法是在**封装外先补一小块加宽的 GND 铜**
（例如从 U5.10/U5.11 的 bridge 中点向 y≈67.3 引一条 0.4~0.5 mm 宽的短铜，或把 U5.5 那条
0.15 mm 短桩局部加宽到 0.4~0.5 mm），让 0.4/0.20 过孔有落点，再落孔接 In1 —— 而不是继续
在细条里找位置（已证明无解）。

**第二十轮（2026-09-18，换思路：先扫"能落孔的位置"）**

新增 `tools/find_via_spot.py`（扫描指定窗口内 0.4/0.20 过孔的**合法格**并按簇输出）。
在 U5 周围 `(28.0,65.5)-(31.0,72.0)` 扫到一个 12 格的合法簇：**中心 (28.70,71.85)**。

但实测该点**在 `USB_VBUS_DET` 竖向段（x=29.30，y 70.6~72.3）的西侧**，而 U5.5 的短桩在
东侧 —— 要接过去必须**跨过 VBUS_DET** ✗（尝试用 `(29.73,71.95)→(29.73,72.55)→(28.60,72.55)
→(28.70,71.85)` 绕行时，DRC 立刻报 `tracks_crossing` + 0.075 mm 间距违规）。

结论：`USB_VBUS_DET` 现在以一条 **"L"形**（竖 x=29.30 + 横 y=72.30）把 U5.5 的短桩包在一个
口袋里，而口袋里的铺铜本身是孤立块。**真正要改的是 `USB_VBUS_DET` 进 U5.4 的走法**
——让它从**西侧**（贴 U5.3 那一线）进入 U5.4，把东侧口袋打开，U5.5 才能接到 (28.70,71.85)
那颗合法过孔。

本轮该尝试已**回退**（不留交叉/悬空），板子保持
**unconnected = 2 / DRC 0 Error 0 Warning / Non-GND = 0**。

**第二十一轮（2026-09-18，收官：unconnected = 0 ✅）**

前二十轮把 255 项未连接收敛到 2 项（都在 U5 的地焊盘附近）。本轮先做**几何判定**：新增三个
只读工具 ——

| 工具 | 作用 |
|---|---|
| `tools/inspect_area.py` | 打印窗口内全部焊盘 / 走线 / 过孔坐标，并可判断某点是否落在实铺铜里、或输出铺铜栅格图 |
| `tools/mask_map.py` | 用布线器**同一套净空引擎**打印"可走线 `+` / 可落孔 `#`"掩膜图，一眼看出哪里还有走廊 |
| `tools/render_area.py` | 把窗口内的铺铜 / 走线 / 焊盘 / 过孔渲染成 PNG（人工核对用） |

结论：这两个铜块**没有任何 0.4/0.20 过孔落点**（掩膜图与逐点 `probe_via_at` 双重确认），
必须先让开"封死它们的铜"：

| 位置 | 封死它的铜 | 处理 |
|---|---|---|
| `U5.10 + U5.11`（北排，跨接铜 `(29.3,68.675)-(29.7,68.675)`） | B.Cu `BAT_BUS` 0.8 mm 主干正好盖住唯一可落孔的格 `(29.5,68.675)`（主干占 y 67.8~68.6）；北面 R11.1 焊盘 + I2C_SCL 竖段把走廊压到 0.357 mm，南面 U5.9/U5.12 焊盘间隙只有 0.2 mm | **把 BAT_BUS 主干在 U5 上方"拱"起来**：`(31.10,68.20)→(30.45,68.20)→(30.00,67.75)→(28.90,67.75)→(28.45,68.20)→(24.50,68.20)`，**宽度仍 0.8 mm、两端端点不动**（南北各一段 45° 折角，平段抬到 y=67.75）→ 落 **0.4/0.20 GND 过孔 (29.50,68.675)**：钻孔在 U5.10/U5.11 之间、位于焊盘上沿以北 0.225 mm，铜只压在两个焊盘上（实体搭接，不是焊盘中心塞孔） |
| `U5.5`（南排，短桩末端 `(29.734,70.693)`） | `USB_VBUS_DET` 进 U5.4 的竖段（x=29.30）把它和西侧主铺铜隔开；3V3_MAIN 的 In2 主干在 y≥71.2 又回到 0.85/1.20 mm，把口袋东侧夹到无孔可落 | **把既有 0.5 mm 窄颈多延长 1.4 mm**：`(30.60,71.20)-(30.60,71.55)` 0.85→0.5、`(30.60,71.55)-(30.60,75.10)` 1.20→0.5 至 y=72.60 → 在 `(29.80,71.10)` 落 **0.4/0.20 GND 过孔**，并用两段 0.15 mm F.Cu（`(29.7343,70.66)-(29.7343,71.10)` 与 `(29.7343,71.10)-(29.80,71.10)`）把 U5.5 的短桩接到该过孔 |

结果：

```text
DRC  0 Error 0 Warning
     unconnected = 0 ✅（255 → 68 → 62 → 59 → 50 → 35 → 18 → 8 → 2 → 0）
tools/check_routing.py ：线-线 / 线-过孔间距 0 问题（独立几何校验）
tools/gnd_components.py：GND 铜 = 1 个分量（MAIN_GND，含 83 个 GND 焊盘 / 160 个 GND 过孔）
```

新增脚本 `tools/fix_u5_gnd_round21.py`（可重复执行：只改 `routing/routing.json`，先备份
`routing/_pre_round21.json`，再由 `tools/apply_routing.py` 重建板子）。检查点：
`esp32-board-v1.1_unconnected_zero_20260918.kicad_pcb` +
`routing/routing_unconnected_zero_20260918.json`。

> 取舍说明：BAT_BUS 只改了 **B.Cu 走线的 y 位置**（0.45 mm），宽度、端点、In2 参考面都没动，
> 电池主干下游布线不受影响；3V3_MAIN 只是把**已经存在的** 0.5 mm 窄颈延长 1.4 mm（该段本就是
> 瓶颈宽度），电性上约 +0.7 mΩ。两颗新过孔都是板级最小尺寸 0.4/0.20（`min_via_diameter = 0.4`），
> 且都不是"焊盘中心塞孔"。
