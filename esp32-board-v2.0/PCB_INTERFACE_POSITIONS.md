...
|---|---|---|---|---|---|---|
| USB-C 插座 | **J1** | GCT USB4105-xx-A，USB-C 2.0 16P 卧式贴装 | (24.000, 81.330) | 0° | 18.680 – 29.320 | 76.570 – 85.510 |
...
| 项 | 数值 |
|---|---|
| 器件 | GCT `USB4105-15-A-120`（USB-C 2.0 Sink，16P，卧式顶部安装） |
...
| 供电 | 3V3_MAIN（TPS63070 输出），模组 3V3 引脚就近去耦（C1 22 µF + C2/C3/C4/C5） |
| 复位 / 烧录 | SW1 = RESET（`RESET_N`）、SW2 = BOOT（`BOOT`，GPIO0） |
| 天线 | 模组天线位于板的**上边缘**，对应上方 X 16 – 39 / Y 0 – 6 的天线禁布区 |
| 调试 | 本版**不设测试点**，GPIO38/39 与 UART（GPIO43/44）为显式 No Connect |
...
| 21 | IO13 | `SPI_MISO` | SPI MISO（墨水屏与 TF 共用） |
...
| 接口 | 4 线 SPI（SCLK / MOSI / MISO / CS#）+ D/C# + RES# + BUSY，与 TF 卡共用 SPI 总线、独立片选 |
...
| 输入保护 | 每路各一颗 2 A PTC（F2 / F3）+ 0.1 µF，汇入公共 `BAT_BUS` |
...
| 存储 | microSD（SPI 模式）：SCLK / MOSI / MISO 与墨水屏共用（经 0 Ω R33 / R34 / R35），`TF_CS_N`（GPIO14）独立片选，`TF_CD_N`（GPIO2）卡检测，10 kΩ 上拉 |
...
| # | 事项 | 现状 / 结论 |
|---:|---|---|
| 1 | USB-C 本体超出板下边缘 **1.005 mm** | 已按 1 mm 外凸设计，外壳开孔需对应预留 |
